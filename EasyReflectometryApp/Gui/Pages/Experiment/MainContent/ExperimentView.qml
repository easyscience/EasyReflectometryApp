...
Rectangle {
    ...
    EaCharts.QtCharts1dMeasVsCalc {
        ...
        // Hidden anchor series to keep the linear axisX alive when all visible series use axisXLog
        LineSeries {
            id: linearAxisAnchor
            axisX: chartView.axisX
            axisY: chartView.axisY
        ...
        LogValueAxis {
            ...
            property double maxAfterReset: Math.max(Globals.BackendWrapper.plottingExperimentMaxX * 1.1, 1e-5)
            base: 10
            lineVisible: false
            labelsVisible: false
            gridVisible: false
        ...
        // Hidden anchor series to keep axisXLog alive when all visible series use linear axisX
        LineSeries {
            id: logAxisAnchor
            axisX: axisXLog
            axisY: chartView.axisY
        ...
        function switchXAxis() {
            var newAxis = useLogQAxis ? axisXLog : axisX
            for (var i = 0; i < chartView.count; i++) {
                var s = chartView.series(i)
                if (s && s !== linearAxisAnchor && s !== logAxisAnchor) s.axisX = newAxis
            }
            // Toggle axis element visibility (never use .visible to avoid Qt Charts detaching axes)
            ...
            axisXLog.labelsVisible = useLogQAxis
            axisXLog.gridVisible = useLogQAxis
            axisXLog.minorGridVisible = useLogQAxis
            axisXLog.titleVisible = useLogQAxis
            if (useLogQAxis) {
