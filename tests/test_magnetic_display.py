...
class TestMomentArrows:
    """Spin-direction design A4: the moment arrow band on the SLD chart."""

    def test_off_by_default_and_absent_without_magnetism(self, qcore_application):
        plotting = Plotting1d(project_lib=_plain_project(), parent=None)

        assert plotting.sldArrowsVisible is False
        assert plotting.getMagneticLayerMarkers(0) == []
        # Stubs built without __init__ still have the flag.
        assert Plotting1d._sld_arrows_visible is False

    def test_markers_describe_each_magnetic_layer(self, qcore_application):
        plotting = Plotting1d(project_lib=_magnetic_project(theta_m=40.0), parent=None)

        markers = plotting.getMagneticLayerMarkers(0)

        assert len(markers) == 1
        assert markers[0]['phi'] == pytest.approx(130.0)
        assert markers[0]['has_moment'] is True
        # The extent is in the profile's own z frame, not thickness from zero.
        z = plotting._magnetic_sld_profiles(0)['rho_m'].x
        assert z.min() <= markers[0]['z_min'] < markers[0]['z_max'] <= z.max()

    def test_markers_are_cached_until_invalidated(self, qcore_application):
        plotting = Plotting1d(project_lib=_magnetic_project(), parent=None)

        first = plotting.getMagneticLayerMarkers(0)
        assert plotting.getMagneticLayerMarkers(0) is first

        plotting.notifyMagneticProfileChanged()

        assert plotting.getMagneticLayerMarkers(0) is not first

    def test_a_stale_model_index_draws_nothing_rather_than_raising(self, qcore_application):
        plotting = Plotting1d(project_lib=_magnetic_project(), parent=None)

        assert plotting.getMagneticLayerMarkers(7) == []

    def test_toggling_arrows_notifies_without_touching_the_sld_range(self, qcore_application):
        plotting = Plotting1d(project_lib=_magnetic_project(), parent=None)
        magnetic, ranges = [], []
        plotting.magneticProfileChanged.connect(lambda: magnetic.append(True))
        plotting.sldChartRangesChanged.connect(lambda: ranges.append(True))

        plotting.setSldArrowsVisible(True)

        assert plotting.sldArrowsVisible is True
        assert magnetic == [True]
        assert ranges == []  # an overlay must never move the y axis

        plotting.setSldArrowsVisible(True)
        assert magnetic == [True]  # idempotent

    def test_arrows_are_not_a_member_of_the_curve_set(self, qcore_application):
        plotting = Plotting1d(project_lib=_magnetic_project(), parent=None)

        plotting.setSldArrowsVisible(True)

        assert 'arrows' not in plotting.MAGNETIC_SLD_CURVES
        assert 'arrows' not in plotting.visibleSldCurves


class TestMomentArrowsQml:
    """What the QML sources must hold to (the app has no QML test harness)."""

    @staticmethod
    def _chart() -> str:
        return (ROOT / 'EasyReflectometryApp' / 'Gui' / 'SldChart.qml').read_text(encoding='utf-8')

    def test_the_band_is_a_sibling_overlay_that_cannot_move_the_plot_area(self):
        chart = self._chart()

        # Height 0 without bands, so a non-magnetic chart keeps its geometry.
        assert 'height: headerHeight + root.arrowBands.length * root.arrowBandHeight' in chart
        assert 'x: chartView.x + chartView.plotArea.x' in chart
        assert 'width: chartView.plotArea.width' in chart
        # The ChartView is pushed down by the overlay, not given chart margins.
        assert ('anchors.topMargin: EaStyle.Sizes.toolButtonHeight - EaStyle.Sizes.fontPixelSize - 1'
                ' + arrowOverlay.height') in chart
        assert 'chartView.margins' not in chart

    def test_arrows_remap_on_every_geometry_change_not_only_on_new_data(self):
        chart = self._chart()

        assert 'chartView.plotArea.x, chartView.plotArea.width' in chart
        assert 'root.chartAxisX.min, root.chartAxisX.max' in chart
        assert 'root.chartAxisX.reverse' in chart
        assert 'onGeometryChanged: Qt.callLater(place)' in chart
        # Reverse z follows mapToPosition; z is never mirrored by hand.
        assert 'chartView.mapToPosition(Qt.point(marker.z_center' in chart

    def test_the_band_model_is_assigned_as_a_new_array(self):
        chart = self._chart()

        assert 'arrowBands = bands' in chart
        assert 'arrowBands.push(' not in chart
        # Values change without the count changing, so it is rebuilt every refresh.
        assert 'rebuildArrowBands()' in chart

    def test_dense_stacks_skip_colliding_arrows_and_say_how_many(self):
        chart = self._chart()

        assert 'if (x - lastX < root.arrowGlyphSize)' in chart
        assert 'missed.push(marker.label)' in chart
        assert 'qsTr("Too close to draw at this zoom: %1").arg(band.skipped.join(\', \'))' in chart

    def test_bands_are_capped_and_carry_the_model_identity(self):
        chart = self._chart()

        assert 'readonly property int maxArrowBands: 2' in chart
        assert 'qsTr("+%1 models").arg(root.hiddenArrowBands)' in chart
        assert 'color: band.bandData.color' in chart
        assert 'text: band.bandData.label' in chart

    def test_the_checkbox_is_its_own_flag_and_off_by_default(self):
        control = (ROOT / 'EasyReflectometryApp' / 'Gui' / 'MagneticProfileControl.qml').read_text(encoding='utf-8')

        assert 'checked: Globals.BackendWrapper.plottingSldArrowsVisible' in control
        assert 'plottingSetSldArrowsVisible(checked)' in control

    def test_wrapper_and_mock_expose_the_new_contract(self):
        wrapper = (ROOT / 'EasyReflectometryApp' / 'Gui' / 'Globals' / 'BackendWrapper.qml').read_text(encoding='utf-8')
        mock = (ROOT / 'EasyReflectometryApp' / 'Backends' / 'Mock' / 'Plotting.qml').read_text(encoding='utf-8')

        assert 'plottingSldArrowsVisible' in wrapper
        assert 'plottingSetSldArrowsVisible' in wrapper
        assert 'plottingGetMagneticLayerMarkers' in wrapper
        for name in ('sldArrowsVisible', 'setSldArrowsVisible', 'getMagneticLayerMarkers'):
            assert name in mock
